...
ROM="${2##*/}"
CORE=${3,,}
#echo "platform: ${PLATFORM} rom: ${ROM} core: ${CORE}"
SHADERSET=0
LOGSDIR="/tmp/logs"
LOGFILE="exec.log"
...
if [ "${BEZEL}" != "false" ] && [ "${BEZEL}" != "none" ] && [ "${BEZEL}" != "0" ] && [ ${SystemViewport[${PLATFORM}]+_} ]; then
        ...
        #echo "Bezel system: ${BEZEL_SYSTEM}"
        BEZEL_SYSTEM_PNG=${path}/systems/${BEZEL_SYSTEM}.png
        #echo "Bezel system png: ${BEZEL_SYSTEM_PNG}"
        ...
        if [[ -f "${game_cfg}" ]]; then
                #echo "game config file exists: ${game_cfg}"
                contents=$(cat "${game_cfg}" |  awk '{$1=$1};1')
                #echo "contents: ${contents}"
                BEZEL_SYSTEM_PNG=${path}/systems/${BEZEL_SYSTEM}/games/${contents}
        fi
        #echo "bezel png: ${BEZEL_SYSTEM_PNG}"
        if [[ -f "${BEZEL_SYSTEM_PNG}" ]]; then
                ...
                if [[ -d "${path}/systems/${BEZEL_SYSTEM}/overlays/" ]]; then
                        for overlay_png in $(ls -1 ${path}/systems/${BEZEL_SYSTEM}/overlays/*.png) ; do
                                ...
                                #echo "overlay name: ${overlay_name} overlay setting: ${overlay_setting}"
                                if [[ "${overlay_setting}" == "none" ]];then
                                        continue
                                fi
